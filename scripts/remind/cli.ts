// npm run remind -- [--week N --boundary early|late] [--send] [--dry-run] [--yes]
//
// The week reminder: one message, To the admin's mailbox and Bcc every
// address on the live roster, six hours before a week's early or late
...
//
//   npm run remind                           the boundary due now, as one Bcc draft (nothing due: says so and exits)
//   npm run remind -- --week 2 --boundary late    a named boundary, for a hand run
...
function parseArgs(argv: string[]): Args {
  const a: Args = { week: null, boundary: null, send: false, dryRun: false, yes: false };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (x === "--week") {
      ...
      const v = argv[++i];
      if (v !== "early" && v !== "late") throw new Error("--boundary must be early or late");
      a.boundary = v;
  ...
  if ((a.week === null) !== (a.boundary === null)) throw new Error("--week and --boundary go together.");
  return a;
}
...
async function main(): Promise<void> {
  ...
  // ---- which boundary: the weeks table and the clock, or a named one
  let b: Boundary | null;
  if (args.week !== null && args.boundary !== null) {
    b = findBoundary(weeks, args.week, args.boundary);
    if (!b) throw new Error(`Week ${args.week} has no ${args.boundary} deadline.`);
  ...
  console.log(`${key}: ${b.kind} deadline ${formatEt(b.deadlineIso)}.`);
  ...
  // ---- send: only through sendWeekReminder, once per boundary
  if (args.send) {
